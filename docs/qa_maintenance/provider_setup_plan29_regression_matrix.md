...
| Credential summary mapping | Canonical daemon fields `configured` and `masked_key_hint` render a stored API key, while a configured OAuth summary renders Connected Account rather than Disconnected; legacy `has_secret` and `masked_secret` remain read-only migration aliases. |
...
|---|---|
| Agent unit tests | Readiness promotion waits for verified cache state. |
| Agent interface tests | Templates/instance commands and `model.snapshot` status are exercised through `SanadProtocolBridge`. |
| Client bloc/widget tests | Template loading no longer relies on legacy fallback, snapshot DTOs carry instance `status`, and the shared flow is covered in bounded-overlay and unbounded-Settings layouts. |
