...
63. **Legacy false-block repair**: a `blocked` work item left by the older interactive-restart bug returns to `waiting` when every executing tool call still has an unresolved checkpoint, and its stale interruption notice is deleted.
