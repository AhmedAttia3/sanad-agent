...
- **Temporary hotfix key:** until transport ownership is narrowed further, the dedupe key is `event_id + transport` rather than `event_id` alone. This prevents a cloud copy from suppressing the corresponding local copy for the same logical event.
...
- **No same-transport re-broadcast loop:** the deduplicator drops only repeated deliveries on the same transport. Local and cloud copies are both allowed through during this temporary mitigation.
