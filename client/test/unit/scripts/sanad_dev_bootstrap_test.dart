...
void main() {
  ...
  setUp(() async {
    ...
    for (final path in [
      ...
      await Directory('${fixture.path}${Platform.pathSeparator}$path').create();
    }
    await File('${fixture.path}${Platform.pathSeparator}.fvmrc').writeAsString(
      '{"flutter":"3.41.9"}',
    );
    await File('${fixture.path}${Platform.pathSeparator}agent${Platform.pathSeparator}pubspec.lock')
        .writeAsString('agent-lock');
    await File('${fixture.path}${Platform.pathSeparator}client${Platform.pathSeparator}pubspec.lock')
        .writeAsString('client-lock');
    await File('${fixture.path}${Platform.pathSeparator}scripts${Platform.pathSeparator}sanad_dev.dart')
        .writeAsString('');
    await File('${fixture.path}${Platform.pathSeparator}scripts${Platform.pathSeparator}sanad-dev')
...
''');
  ...
  test('fresh setup runs Flutter then both dependency stages', () async {
    ...
    expect(invocations[1], contains('agent|dart pub get'));
    expect(invocations[2], contains('client|flutter pub get'));
  ...
  test('second setup skips dependency resolution with a valid stamp', () async {
    ...
    expect(result.stdout, contains('Resolving package dependencies'));
    expect(result.stdout, contains('skipped'));
    final invocations = await calls.readAsLines();
  ...
  test('failed stage blocks dependent setup and runtime launch', () async {
    ...
    expect(invocations.last, contains('agent|dart pub get'));
  ...
  test('no arguments completes setup then launches run all', () async {
    final result = await runBootstrap(const []);

    expect(result.exitCode, 0);
    expect((await calls.readAsLines()).last, contains('sanad_dev.dart run all'));
  });
  ...
  test('Windows setup executes all stages with a user-scoped shim', () async {
    ...
    expect(
      File('${fixture.path}${Platform.pathSeparator}SanadDev${Platform.pathSeparator}bin${Platform.pathSeparator}sanad-dev.cmd').existsSync(),
      isTrue,
    );
  }, skip: !Platform.isWindows);
